...
class PlaylistViewModel : ViewModel() {
    ...
    fun seedData() {
        ...
        playlists.forEach { (playlistId, canciones) ->
            val playlistRef = db.collection("playlists").document(playlistId)

            // Revisa si ya existen canciones antes de insertar
            playlistRef.collection("canciones")
                .get()
                .addOnSuccessListener { snapshot ->
                    if (snapshot.isEmpty) { // 👈 solo carga si no hay nada
                        canciones.forEach { (titulo, artista) ->
                            ...
                            playlistRef.collection("canciones").add(nuevaCancion)
                        }
                    }
                }
        }
